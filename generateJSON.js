...
function cutModuleFromJSON() {
    shell.cd("results/runtime");
    shell.exec("sed -i '1d' modules.json");
    shell.exec("sed -i '$d' modules.json");
    shell.exec("sed -n '1, 9p' modules.json> modules/android.json");
    shell.exec("sed -n '10, 18p' modules.json> modules/html5.json");
    shell.exec("sed -n '19, 27p' modules.json> modules/ios.json");
    shell.exec("sed -n '28, 36p' modules.json> modules/linux.json");
    shell.exec("sed -n '37, 45p' modules.json> modules/linuxYYC.json");
    shell.exec("sed -n '46, 54p' modules.json> modules/mac.json");
    shell.exec("sed -n '55, 63p' modules.json> modules/macYYC.json");
    shell.exec("sed -n '64, 72p' modules.json> modules/ps4.json");
    shell.exec("sed -n '73, 81p' modules.json> modules/switch.json");
    shell.exec("sed -n '82, 90p' modules.json> modules/tvos.json");
    shell.exec("sed -n '91, 99p' modules.json> modules/windows.json");
    shell.exec("sed -n '100, 108p' modules.json> modules/windowsuap.json");
    shell.exec("sed -n '109, 117p' modules.json> modules/windowsYYC.json");
    shell.exec("sed -n '118, 126p' modules.json> modules/xboxone.json");
    shell.cd("modules");
    shell.ls('*.json').forEach(function (file) {
        shell.sed('-i', '},', '}', file);
    });
}
...
function cutModule() {
    fs.access("./results/result3.json", fs.constants.F_OK, function (err) {
        if (!err) {
            ...
            let cuttedModule = JSON.stringify(modulesJSON.enclosure.module, null, "\t");
            fs.writeFile("./results/runtime/modules.json", cuttedModule, (err) => {
                cutModuleFromJSON();
                console.log("Cut the module from JSON successfully.");
            });
        } else {
            console.log(err);
        }
